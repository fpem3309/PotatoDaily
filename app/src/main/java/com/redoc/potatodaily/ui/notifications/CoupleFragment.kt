...
import com.redoc.potatodaily.databinding.FragmentCoupleBinding
...
class CoupleFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val dateCoupleCnt = viewModel.getCoupleBoard("dateCouple")
        val anniversaryCnt = viewModel.getCoupleBoard("anniversary")
        val giftCnt = viewModel.getCoupleBoard("gift")
        val conflictCnt = viewModel.getCoupleBoard("conflict")
        val loveCnt = viewModel.getCoupleBoard("love")

        var chart = binding.barChart

        val entries = ArrayList<BarEntry>()
        entries.add(BarEntry(1.0f,dateCoupleCnt!!.toFloat()))
        entries.add(BarEntry(2.0f,anniversaryCnt!!.toFloat()))
        entries.add(BarEntry(3.0f,giftCnt!!.toFloat()))
        entries.add(BarEntry(4.0f,conflictCnt!!.toFloat()))
        entries.add(BarEntry(5.0f,loveCnt!!.toFloat()))

        var barDataSet = BarDataSet(entries,"커플")//데이터셋 초기화 하기
        barDataSet.color = ContextCompat.getColor(context!!,R.color.coupleCharf)
    ...
    inner class MyXAxisFormatter : ValueFormatter(){
        private val days = arrayOf("데이트","기념일","선물","다툼","관계")
